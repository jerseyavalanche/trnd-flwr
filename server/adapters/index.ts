...
async function rssAdapter() {
  ...
  try {
    const feed = (process.env.NEWS_RSS_FEEDS || 'https://rss.nytimes.com/services/xml/rss/nyt/Technology.xml').split(',')[0].trim();
    ...
    const ts = new Date(item.pubDate?.[0] ?? nowIso()).toISOString(); const ageHours = (Date.now() - new Date(ts).getTime()) / 36e5;
    ...
    return { signals: [signal], status: ok(source, `RSS live (${feed})`) };
...
async function githubAdapter() {
  ...
  try {
    ...
    const d = await asJson(`https://api.github.com/search/repositories?q=created:%3E${after}&sort=stars&order=desc&per_page=1`, { headers });
    const r = d.items?.[0]; if (!r) throw new Error('No repo result');
