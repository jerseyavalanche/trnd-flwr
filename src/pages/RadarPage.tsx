    ...
    <>
      <section className="rounded-xl border border-slate-800 bg-slate-900 p-3">
        ...
        <div className="text-xs text-slate-300 grid grid-cols-2 gap-2 mt-2">
          <p>Backend: {data.systemStatus.backend}</p>
          <p>Ingestion: {data.systemStatus.ingestion}</p>
          <p>Storage: {data.systemStatus.storage.ok ? 'ok' : 'error'}</p>
          <p>Model: {data.systemStatus.modelSynthesis.status}</p>
          <p>Last Scan: {data.systemStatus.lastScanTime ? new Date(data.systemStatus.lastScanTime).toLocaleTimeString() : 'none'}</p>
          <p>Failed Sources: {data.systemStatus.failedSourceCount}</p>
        </div>
      </section>

      <section className="rounded-xl border border-slate-800 bg-slate-900 p-3">
        <h2 className="font-semibold">Civilization Regime</h2>
        <p className="text-cyan-300 text-sm mt-1">{data.regime.label}</p>
      </section>

      <section>
        <h2 className="font-semibold mb-2">Rising Themes</h2>
        <div className="space-y-2">{data.themes.slice(0, 6).map((t) => <div key={t.name} className="rounded-lg border border-slate-800 p-3 bg-slate-900"><p className="font-medium">{t.name}</p><p className="text-xs text-slate-400">strength {t.strength} · accel {t.acceleration} · confidence {t.confidence}</p></div>)}</div>
      </section>
